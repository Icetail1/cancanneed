...
LinkGame.prototype = {
  ...
  drawLine: function (callback) {
    ...
    for (var i = 0; i < linkList.length; i++) {
      var x = linkList[i].col === 0 ? 0 : (linkList[i].col === this.cols - 1 ? $('#game').width() : linkList[i].col * 25 - 20);
      var y = linkList[i].row === 0 ? 0 : (linkList[i].row === this.rows - 1 ? $('#game').height() : linkList[i].row * 25 - 20);
      coordinate.push([x, y]);
    }
...
var Url2 = "IceCan " + self.score.toString(16);//获取span框的文本内容
console.log(Url2);
